...
/*
Source: https://github.com/Tom-Alexander/regression-js/blob/master/src/regression.js
Modified so I don't use node.js or any packaging. I removed the wrapper and exports ,and truend it into indivudal functions.
*/
...
function linear(data, options = DEFAULT_OPTIONS) {
    ...
    return {
      ...
      string: intercept === 0 ? `y = ${gradient.toExponential(3)}x` : `y = ${gradient.toExponential(3)}x + ${intercept.toExponential(3)}`,
    };
  };

function exponential(data, options = DEFAULT_OPTIONS) {
    ...
    return {
      ...
      string: `y = ${coeffA.toExponential(3)}e^(${coeffB.toExponential(3)}x)`,
      r2: round(determinationCoefficient(data, points), options.precision),
    };
  };

function logarithmic(data, options = DEFAULT_OPTIONS) {
    const sum = [0, 0, 0, 0];
    ...
    return {
      ...
      predict,
      equation: [coeffA, coeffB],
      string: `y = ${coeffA.toExponential(3)} + ${coeffB.toExponential(3)} ln(x)`,
      r2: round(determinationCoefficient(data, points), options.precision),
    };
  };

function power(data, options = DEFAULT_OPTIONS) {
    const sum = [0, 0, 0, 0, 0];
    const len = data.length;
    ...
    return {
      ...
      predict,
      equation: [coeffA, coeffB],
      string: `y = ${coeffA.toExponential(3)}x^${coeffB.toExponential(3)}`,
      r2: round(determinationCoefficient(data, points), options.precision),
    };
  };
...
function polynomial(data, options = DEFAULT_OPTIONS) {
    ...
    for (let i = coefficients.length - 1; i >= 0; i--) {
      if (i > 1) {
        string += `${coefficients[i].toExponential(3)}x^${i} + `;
      } else if (i === 1) {
        string += `${coefficients[i].toExponential(3)}x + `;
      } else {
        string += coefficients[i].toExponential(3);
      }
    }
