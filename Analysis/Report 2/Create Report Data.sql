...
FROM
"PI"."PIInterpArchive" cur  INNER JOIN emsTags t ON cur."tag" = t."TAGID_PI"
WHERE cur."time" between ( TO_TIMESTAMP('20220101 000000', 'YYYYMMDD HH24MISS') - interval '2 minute') 
			 AND TO_TIMESTAMP('20220101 000000', 'YYYYMMDD HH24MISS') + interval '1 month 1 minute')
...
FROM emsTimes WHERE time between '20220101 000000' AND TO_TIMESTAMP('20220101 000000', 'YYYYMMDD HH24MISS')  + interval '1 month'),
...
,final as (SELECT *, case when rampRateUnfiltPrev > 0 or rampRateUnfiltNext > 0 
		then case when rampRateUnfilt > "MAXRAMP" 
			 then null 
				...
				and ((EXTRACT(month FROM time) = 9 and EXTRACT(year FROM time) = 2020) or unit_id = 'TC2' or true)--(BlockHigh <> 1 and BlockLow <> 1))
				then rampRateUnfilt 
				else null end end 
		else null end as rampRateUnfiltFinal FROM step1)
,emsData as (SELECT time, unit_id, SetPoint, PrevSetPoint, Prev2SetPoint, "MATCHTARGETRAMP", RampRate, ControlMode, BlockHigh, BlockLow, rampRateUnfilt, rampRateUnfiltPrev, rampRateUnfiltNext, rampRateUnfiltFinal, 
case when rampRateUnfiltFinal > 0 
	...
	else null end as ActualRampPct, prevActGrossGen, Prev2ActGrossGen, NextActGrossGen, "MAXLOAD", ActGrossGen, "MINREPORTINGGEN"
FROM final)
SELECT * FROM emsData
--ORDER BY time